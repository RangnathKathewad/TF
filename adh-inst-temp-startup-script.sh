...
echo "Cloning Beyond compose repo for devops branch"
token=$(gcloud secrets versions access latest --secret=github-api-tokens)
su - ubuntu -c "git clone --branch release/production-280622 https://${token}@github.com/med-and-beyond/beyond-compose.git"
su - ubuntu -c "cd beyond-compose && gcloud auth configure-docker -q && make pull && make up"
